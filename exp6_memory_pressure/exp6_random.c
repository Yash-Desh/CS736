...
typedef struct {
    ...
    size_t    start;
    size_t    length;
    size_t   *perm;          // NEW: random permutation of [0, length)
...
void *worker(void *arg) {
    ...
    // Pin to chosen node (always 0 here unless overridden by argv[4])
    ...
    // For each iteration, walk the chunk in a random index order
    // given by targ->perm, to break spatial locality and prefetching.
    for (size_t r = 0; r < iters; r++) {
        double local_sum = 0.0;
        for (size_t j = 0; j < targ->length; j++) {
            size_t idx = targ->perm[j];  // random index in [0, length)
...
int main(int argc, char **argv) {
    ...
    printf("=== Experiment 6: Node pressure & fallback (RANDOM ACCESS) ===\n");
    ...
    // Fault in pages once (sequential) so you separate page placement from access pattern
    ...
    if (!threads || !targs) {
        fprintf(stderr, "calloc failed for threads/targs\n");
        return 1;
    }

    pthread_barrier_init(&start_barrier, NULL, num_threads);

    // Seed RNG once for permutation generation
    ...
    for (int t = 0; t < num_threads; t++) {
        ...
        // --- NEW: build a random permutation of indices for this thread ---
        targs[t].perm = malloc(chunk * sizeof(size_t));
        if (!targs[t].perm) {
            fprintf(stderr, "malloc failed for perm array of thread %d\n", t);
            return 1;
        }
        ...
        for (size_t i = chunk - 1; i > 0; i--) {
            ...
            size_t tmp = targs[t].perm[i];
            targs[t].perm[i] = targs[t].perm[j];
            targs[t].perm[j] = tmp;
        }

        printf("Thread %2d pinned to node %d (start=%zu, len=%zu, random access)\n",
