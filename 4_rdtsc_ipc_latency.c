...
static int run_size(size_t msg, int warmup, int iters, int cpu, double tsc_hz)
{
  ...
  if (pid == 0)
  {
    ...
    {
      if (read_full(p_ab[0], rcv_buf, msg) != 0)
        _exit(2);
      if (write_full(p_ba[1], rcv_buf, msg) != 0)
        _exit(3);
    }
    close(p_ab[0]);
    close(p_ba[1]);
  ...
  {
    if (write_full(p_ab[1], snd_buf, msg) != 0)
      return -1;
    if (read_full(p_ba[0], rcv_buf, msg) != 0)
      return -1;
  }
...
int main(int argc, char **argv)
{
  ...
  if (argc > 1)
    target_bytes = atof(argv[1]); // bytes per size
  if (argc > 2)
    warmup = atoi(argv[2]);
  if (argc > 3)
    cpu = atoi(argv[3]);
  ...
  for (int i = 0; i < nsizes; i++)
  {
    ...
    if (iters < min_iters)
      iters = min_iters;
    if (run_size(msg, warmup, iters, cpu, hz) != 0)
      return 1;
  }
  return 0;
}
